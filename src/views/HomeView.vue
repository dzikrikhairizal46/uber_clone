<template>
  <div id="Home" class="flex flex-col h-screen">
    <div class="m-3 mb-auto">
      <div class="m-3 mb-auto">
        <div class="my-20">
          <img width="140" src="img/uber/uber2.svg" alt="">
        </div>
      </div>
      <div class="grid grid-cols-2 gap-2 my-3">
        <ServiceSelectLarge
          @click="router.push('/directions')"
          text="Ride"
          imageWidth="74"
          image="ride"
        />
        <ServiceSelectLarge
          text="Food"
          imageWidth="74"
          image="food"
          :promo="true"
        />
      </div>
      <div class="grid grid-cols-4 gap-3 my-3">
        <ServiceSelectSmall
          text="Bike"
          image="2wheels"
        />
        <ServiceSelectSmall
          text="Transit"
          image="transit"
        /><ServiceSelectSmall
          text="Reserve"
          image="reserve"
        /><ServiceSelectSmall
          text="Travel"
          image="travel"
        />
      </div>
      <div
      @click="router.push('/directions')" 
      class="w-full bg-custom-color h-14 rounded-full items-center flex p-3">
        <MagnifyIcon :size="40"/>
        <div class="ml-3 text-xs font-semibold text-gray-700">
          Enter pick-up point
        </div>
      </div>
    </div>
    <footer class="w-full z-50 flex justify-around p-1 mb-2 border-t-2 border-gray-200">
      <div class="grid place-items-center">
        <HomeIcon :size="30" fillColor="#1f1e1e"/>        
        <div class="w-full -mt-1 text-xs text-center text-gray-800 font-semibold">
          Home
        </div>
      </div>
      <div @click="router.push('/directions')" class="grid place-items-center">
        <MapMarkIcon :size="30" fillColor="#b3b1b1"/>        
        <div class="w-full -mt-1 text-xs text-center text-gray-400 font-semibold">
          Destination
        </div>
      </div>
    </footer>
  </div>
</template>

<script setup>
  import { useRouter } from 'vue-router'
  import { useDirectionStore } from '@/store/direction-store' 
  import ServiceSelectLarge from "../components/ServiceSelectLarge.vue"
  import ServiceSelectSmall from "../components/ServiceSelectSmall.vue"
  import MagnifyIcon from "vue-material-design-icons/Magnify.vue"
  import HomeIcon from "vue-material-design-icons/Home.vue"
  import MapMarkIcon from "vue-material-design-icons/MapMarker.vue"
  import { onMounted } from 'vue'
  
  const router = useRouter()
  const direction = useDirectionStore()

  onMounted(() => {
    direction.pickup = ''
    direction.destination = ''
  })
</script>
<style lang="scss" scoped>
  #Home{
    .bg-custom-color{
      background-color: rgb(237, 237, 237);
    }
  }
</style>